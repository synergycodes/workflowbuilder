...
**Changesets for `@workflowbuilder/temporal` follow the SDK rules.** The package is released, so every consumer-visible change needs its own changeset. `pr-check.yml` warns when `packages/execution-core` or `packages/types` change without a changeset for it.
