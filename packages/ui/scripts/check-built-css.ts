 ...
 * past review or a build-tool transform introduces it. The rule: `var()`'s
 * first argument must be a `<custom-property-name>` (a dashed-ident) - never
 * another function. Browsers silently invalidate `var(var(--foo))` and fall
 * back, which is how WB-222 shipped a wrong snackbar icon color.
 *
 * This is the only line of defense for this bug class today; a source-level
 * lint rule (e.g. a stylelint `no-invalid-var`) would catch it earlier but
