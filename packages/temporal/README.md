...
Because this is an Update and not a signal, the caller gets a synchronous answer. The first verdict for a node wins: a second one is rejected with an `ApplicationFailure` of type `verdict_already_delivered`, and a verdict for a run that has already closed fails at the server. Cancelling a parked run closes it as `cancelled`, with `execution_cancelled` following the node's `node_waiting` and no `node_failed` recorded for the node that was waiting.
