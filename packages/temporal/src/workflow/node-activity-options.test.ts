...
describe('assertNodeActivityProfiles', () => {
  // The reason this runs eagerly: a profile checked at scheduling time throws inside
  // the node activity, where the graph's errorPolicy can absorb it and close the run
  // as completed. A configuration mistake would look like a node problem.
  ...
  it('rejects a duration Temporal would not parse, naming the config path', () => {
    ...
    expect(() => assertNodeActivityProfiles(profiles as unknown as NodeActivityProfiles)).toThrow(
      /nodeActivityProfiles\["test\/step"\]\.startToCloseTimeout must be a duration/,
    );
  });
