...
| `Error.cause` chain traversal            | Yes             | ✅ Safe — `extractDeepestError` walks the chain to surface the deepest message. Walk is bounded by `MAX_CAUSE_DEPTH = 16`, so a cyclic chain produced by a buggy adapter (`a.cause = b; b.cause = a`) cannot spin the workflow. Bound is part of the contract — see rule 8.                 |
