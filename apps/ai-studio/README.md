...
- The person decides in the node's properties sidebar: the editor's own form over `decisionRequest.schema`, filled from the proposal source's output, with read-only fields disabled and only the changed editable fields sent as `edits`. From the moment the backend starts a run until Reset the canvas is read-only, so it stays the graph the run executes.
