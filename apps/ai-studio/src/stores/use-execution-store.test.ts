...
describe("use-execution-store: the run's outcome", () => {
  const outcome = { value: 'rejected', resolvedBy: 'human', nodeId: 'human-1' };
  ...
  it('records the outcome execution_completed carries, and closes the run', () => {
    applyEvent(event({ type: 'execution_started', payload: { workflowId: 'wf-1' } }));
    applyEvent(event({ type: 'execution_completed', payload: { outcome } }));

    expect(useExecutionStore.getState().status).toBe('completed');
    expect(useExecutionStore.getState().outcome).toEqual(outcome);
  });

  it('leaves the outcome undefined when the run completes without one', () => {
    applyEvent(event({ type: 'execution_started', payload: { workflowId: 'wf-1' } }));
    applyEvent(event({ type: 'execution_completed' }));

    expect(useExecutionStore.getState().status).toBe('completed');
    expect(useExecutionStore.getState().outcome).toBeUndefined();
  });

  it('a snapshot replayed after a reload carries the outcome', () => {
    const events = [
      event({ type: 'execution_started', payload: { workflowId: 'wf-1' } }),
      event({ type: 'node_waiting', nodeId: 'human-1' }),
      event({ type: 'node_completed', nodeId: 'human-1', payload: { output: { action: 'reject' } } }),
      event({ type: 'execution_completed', payload: { outcome } }),
    ];
  ...
  it('a new run and a reset both start without the previous outcome', () => {
    applyEvent(event({ type: 'execution_completed', payload: { outcome } }));
