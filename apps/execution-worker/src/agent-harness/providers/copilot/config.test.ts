...
import { parseCopilotConfig, parseCopilotRunConfig } from './config';
...
describe('parseCopilotConfig', () => {
  ...
  test('clamps the rungs the shared ladder has and the SDK does not', () => {
    // Copilot's SDK offers 'max' but not the ladder's outer rungs beyond it,
    // so 'persistent'/'ultra' clamp down to 'max' and 'minimal' clamps up to
    // 'low' rather than being dropped.
...
describe('parseCopilotRunConfig', () => {
  test('accepts a run config using only known keys', () => {
    expect(parseCopilotRunConfig({ model: 'gpt-5', modelReasoningEffort: 'high' })).toEqual({
      model: 'gpt-5',
      modelReasoningEffort: 'high',
    });
  });
  ...
  test('rejects an unknown key rather than silently dropping it', () => {
    expect(() => parseCopilotRunConfig({ bogusField: 'x' })).toThrow(InvalidProviderRunConfigError);
  });

  test('rejects an invalid modelReasoningEffort value', () => {
    expect(() => parseCopilotRunConfig({ modelReasoningEffort: 'not-a-rung' })).toThrow(InvalidProviderRunConfigError);
  });

  test('rejects a non-boolean useLoggedInUser', () => {
    expect(() => parseCopilotRunConfig({ useLoggedInUser: 'yes' })).toThrow(InvalidProviderRunConfigError);
  });

  test('rejects an invalid logLevel', () => {
    expect(() => parseCopilotRunConfig({ logLevel: 'verbose' })).toThrow(InvalidProviderRunConfigError);
  });

  test('rejects a blank model string', () => {
    expect(() => parseCopilotRunConfig({ model: '   ' })).toThrow(InvalidProviderRunConfigError);
  });
});
