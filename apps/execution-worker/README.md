...
- **Retries per failure:** an executor throwing `PermanentNodeExecutionError` stops on its first attempt; `TransientNodeExecutionError` retries within the profile's limit. An unclassified throw keeps today's behavior — most reference executors have not been classified yet; `human-decision` is the first that has.
...
- **Deploys that widen the resolution envelope:** the update validator lives in the worker's workflow bundle and refuses an unknown resolution key. A backend that starts sending a new key before the workers accept it gets `verdict_malformed`, which the decision endpoint answers with 500. Order: migrations, then workers, then the backend. The `outcome` key shipped this way.
