...
- **Tool loop inside one Activity:** the AI agent's web-search loop (up to 4 steps) runs entirely inside the node's `executeNode` Activity, so a retry repeats every model call and search made so far. Acceptable while the only tool is read-only; the reasoning, the cost caps and the point at which a tool must get its own Activity are in [`ai-agent-tool-loop-activity.decision-log.md`](src/activities/ai-agent-tool-loop-activity.decision-log.md).
