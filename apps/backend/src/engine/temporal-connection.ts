...
//
// Duplicated by design in apps/execution-worker/src/engines/temporal/temporal-connection.ts:
// the two SDKs type their options separately (the client accepts a function for
// `apiKey`, the worker's native connection only a string), so a shared module would
// have to pick one and cast. Keep the two copies in sync.
...
type TemporalConnectionOptions = Pick<ConnectionOptions, 'tls' | 'apiKey'>;
...
export function buildTemporalConnectionOptions(
  ...
  if (tls !== 'true' && !hasTlsMaterial) {
    // Plaintext — the local-dev default, and what this backend did before.
    return {};
  }
