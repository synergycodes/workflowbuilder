...
The same template renders the canvas node, the palette thumbnail and the drag ghost. In the palette `data` and `layoutDirection` are `undefined` — read them with optional chaining and fall back to defaults. The palette passes `selected` for the drag ghost (the Node Active ring) and `disabled` for entries that cannot be added, for example in read-only mode; forward both to `NodePanel.Root`, `NodeIcon` and `NodeDescription`, otherwise your entry looks draggable while the palette is locked.
