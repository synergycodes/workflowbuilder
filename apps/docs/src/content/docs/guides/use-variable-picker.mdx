...
Add an `schemaOutput` to the node's `PaletteItem`. Each entry describes one output property the node will emit:
...
Without an `schemaOutput`, the node never appears as a group in any downstream picker.
