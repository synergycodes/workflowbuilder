...
A `Chip` is a compact tag in one of two Design System 2.0 treatments: `solid`
on the neutral chip surface, or `outline` with the brand border and accent.
Four sizes (`s`, `m`, `l`, `xl`), an optional prefix icon, and an optional
close affordance.
