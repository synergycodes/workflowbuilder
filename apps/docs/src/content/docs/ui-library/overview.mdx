...
reset, and typography classes - so the only thing left to add is the design
tokens:
...
subpath instead. That only injects the component's own CSS, so import the
global stylesheet once, **before any component**, to establish the cascade-layer
order, plus the tokens:
