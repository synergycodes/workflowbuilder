---
title: Design tokens
description: How @workflowbuilder/ui turns Figma design tokens into the --wb-ds-* CSS custom properties that style every component.
---
...
- **Primitives** become `--wb-ds-colors-*`, `--wb-ds-space-*`, `--wb-ds-radius-*`, etc.
  on `:root` - the raw palette and scales.
- **Tokens/Light** and **Tokens/Dark** become the _semantic_ layer
  (`--wb-ds-ui-*`, `--wb-ds-components-*`, …), each scoped to
