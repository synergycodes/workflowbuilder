...
- Replace `shape="circle"` with `shape="round"` on `Button`, which also gains `shape="square"`. `SegmentPicker` keeps the `Shape` type it always had (`default` or `circle`), so its own `shape` prop needs no change.
